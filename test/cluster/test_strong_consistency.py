...
async def test_timed_out_queries(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 1} AND consistency = 'global'") as ks:
        async with new_test_table(manager, ks, "pk int PRIMARY KEY, v int") as table:
            ...
            await manager.api.enable_injection(s1.ip_addr, "sc_coordinator_wait_before_begin_mutate", one_shot=True)
            batch_fut = cql.run_async(SimpleStatement(f"""
                BEGIN UNLOGGED BATCH USING TIMEOUT 100ms
                INSERT INTO {table} (pk, v) VALUES (19, 23);
                INSERT INTO {table} (pk, v) VALUES (19, 24);
                APPLY BATCH
            """, retry_policy=FallthroughRetryPolicy()))
            await manager.api.wait_for_injection_enter(s1.ip_addr, "sc_coordinator_wait_before_begin_mutate")
            await asyncio.sleep(0.5)
            await manager.api.message_injection(s1.ip_addr, "sc_coordinator_wait_before_begin_mutate")
