...
async def test_basic_write_read(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 2} AND tablets = {'initial': 1} AND consistency = 'global'") as ks:
        ...
        async def collect_metrics():
            ...
            leader_metrics = {
                'scylla_strong_consistency_write_latency_count': leader_metrics_query.get('scylla_strong_consistency_write_latency_count') or 0,
                'scylla_strong_consistency_read_latency_count':  leader_metrics_query.get('scylla_strong_consistency_read_latency_count') or 0,
            }
            non_leader_metrics_query = await manager.metrics.query(non_leader_replica_host.address)
            non_leader_metrics = {
                'scylla_strong_consistency_write_node_bounces': non_leader_metrics_query.get('scylla_strong_consistency_write_node_bounces') or 0,
                'scylla_strong_consistency_read_latency_count': non_leader_metrics_query.get('scylla_strong_consistency_read_latency_count') or 0,
            }
            non_replica_metrics_query = await manager.metrics.query(non_replica_host.address)
            non_replica_metrics = {
                'scylla_strong_consistency_write_node_bounces': non_replica_metrics_query.get('scylla_strong_consistency_write_node_bounces') or 0,
                'scylla_strong_consistency_read_node_bounces':  non_replica_metrics_query.get('scylla_strong_consistency_read_node_bounces') or 0,
            }
