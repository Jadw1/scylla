...
public:
    ...
    virtual future<> set_service_level(sstring service_level_name, qos::service_level_options slo, std::optional<service::group0_guard> guard) const override {
        co_await raft_service_level_distributed_data_accessor::set_service_level(std::move(service_level_name), std::move(slo), std::move(guard));
        co_await _sl_controller.invoke_on_all(&service_level_controller::update_service_levels_from_distributed_data);
    }

    virtual future<> drop_service_level(sstring service_level_name, std::vector<mutation> remove_attribute_mutation, std::optional<service::group0_guard> guard) const override {
        co_await raft_service_level_distributed_data_accessor::drop_service_level(std::move(service_level_name), std::move(remove_attribute_mutation), std::move(guard));
        co_await _sl_controller.invoke_on_all(&service_level_controller::update_service_levels_from_distributed_data);
    }
};
