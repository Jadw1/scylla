...
def test_desc_index_on_collections(cql, test_keyspace):
    ...
    with new_test_table(cql, test_keyspace, "a int primary key, b int, c frozen<set<int>>, d map<int, int>, e list<int>, f set<int>, g list<int>, h set<int>") as tbl:
        indexes = [
...
def test_desc_index_column_quoting(cql, test_keyspace):
    ...
    quoted_names = ['"select"', '"hEllo"', '"x y"', '"hi""hello""yo"', '"""hi"""', '"keys(m)"', '"values(m)"', '"entries(m)"']
    with new_test_table(cql, test_keyspace, "a int primary key, \"some???name\" map<int, int>, " + ', '.join([name + " int" for name in quoted_names])) as tbl:
        indexes = [
            ("idx_1", f"CREATE INDEX idx_1 ON {tbl}(keys(\"some???name\"))"),
        ] + [(f"idx_{n}", f"CREATE INDEX idx_{n} ON {tbl}({name})") for n, name in enumerate(quoted_names, 2)]
...
def test_desc_secondary_index(scylla_only, cql, test_keyspace):
