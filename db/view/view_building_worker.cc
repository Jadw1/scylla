...
void view_building_worker::init_messaging_service() {
    ser::view_rpc_verbs::register_work_on_view_building_tasks(&_messaging, [this] (std::vector<utils::UUID> ids) -> future<std::vector<utils::UUID>> {
        return work_on_tasks(std::move(ids));
    });
}
...
future<> view_building_worker::check_for_aborted_tasks() {
    return container().invoke_on_all([building_state = _vb_state_machine.building_state] (view_building_worker& vbw) -> future<> {
        auto lock = co_await get_units(vbw._lol_state._mutex, 1, vbw._as);
        // TODO: if base table was changed abort immediatelly


        if (!vbw._lol_state._batch) {
            co_return;
        }
        ...
        for (auto& [id, t]: vbw._lol_state._batch->tasks) {
...
future<std::vector<utils::UUID>> view_building_worker::work_on_tasks(std::vector<utils::UUID> ids) {
    auto collect_completed_tasks = [&] {
        ...
        for (auto& id: ids) {
            if (_lol_state.completed_tasks.contains(id)) {
    ...
    auto lock = co_await get_units(_lol_state._mutex, 1, _as);
    // Firstly check if there is any batch that is finished but wasn't cleaned up.
    if (_lol_state._batch && _lol_state._batch->promise.available()) {
        co_await _lol_state.clean_up_after_batch();
    }
    ...
    lock = co_await get_units(_lol_state._mutex, 1, _as);
    co_await _lol_state.update_processing_base_table(_db, building_state, _as);
    // If there is no running batch, create it.
    if (!_lol_state._batch) {
        if (!_lol_state.processing_base_table) {
            on_internal_error(vbw_logger, "view_building_worker::state::processing_base_table needs to be set to work on view building");
        }
        ...
        for (auto& id: ids) {
            auto task_opt = building_state.get_task(*_lol_state.processing_base_table, my_replica, id);
            if (!task_opt) {
        ...
        if (std::ranges::any_of(tasks | std::views::values, [&] (const view_building_task& t) {
            return t.view_id && !_lol_state.flushed_views.contains(*t.view_id);
        })) {
            co_await _lol_state.flush_base_table(_db, *_lol_state.processing_base_table, _as);
        }

        // Create and start the batch
        _lol_state._batch = std::make_unique<batch>(container(), std::move(tasks), *building_state.currently_processed_base_table, my_replica);
        _lol_state._batch->start();
    }

    if (std::ranges::all_of(ids, [&] (auto& id) { return _lol_state._batch->tasks.contains(id); })) {
        throw std::runtime_error(fmt::format(
                "None of the tasks requested to work on is executed in current view building batch. Batch executes: {}, the RPC requested: {}",
                _lol_state._batch->tasks | std::views::keys, ids));
    }
    auto batch_future = _lol_state._batch->promise.get_shared_future();
    lock.return_all();
