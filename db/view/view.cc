...
static future<> announce_with_raft(
    ...
    while (true) {
        ...
        std::vector<canonical_mutation> cmuts(1);
...
future<> view_building_worker::notify() {
    return container().invoke_on_all([] (view_building_worker& vbw) {
        vbw._cond.broadcast();
    });
}

future<> view_building_worker::start_staging_detector() {
    static auto retry_sleep_time = std::chrono::milliseconds(500);
    bool retry;
    while (!_detector_as.abort_requested()) {
        try {
            retry = false;
            co_await register_staging_sstables();
            ...
            vbw_logger.warn("Error in staging sstable detector: {}", std::current_exception());
        }
...
future<> view_building_worker::register_staging_sstables() {
    ...
    co_await dir.process_sstable_dir(sstables::sstable_directory::process_flags{ .sort_sstables_according_to_owner = false });
    const auto& unsorted_ssts = dir.get_unsorted_sstables();
    ...
    for (auto& sst: unsorted_ssts) {
        ...
        for(auto& shard: shards) {
            auto mut = co_await _sys_ks.make_vbc_staging_sstable_mutation(guard.write_timestamp(), this_node_id, shard);
            muts.emplace_back(std::move(mut));
        }
    }
    if (!muts.empty()) {
...
void view_building_worker::init_messaging_service() {
    ...
    ser::view_rpc_verbs::register_process_staging_sstables(&_messaging.local(), [this] (table_id base_id, unsigned shard) -> future<> {
    ...
    ser::view_rpc_verbs::register_abort_vbc_work(&_messaging.local(), [this] (unsigned shard) -> future<rpc::no_wait_type> {
        (void)container().invoke_on(shard, [] (auto& vbw) {
