...
static void describe_index_columns(std::ostream& os, bool is_local, const schema& index_schema, schema_ptr base_schema) {
    ...
    auto index_metadata = base_schema->all_indices().at(secondary_index::index_name_from_table_name(index_schema.cf_name()));
    ...
    } else if (bk_type->is_set() && target_str.contains("keys")) {
