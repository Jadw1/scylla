...
create_table_statement::execute(query_processor& qp, service::query_state& state, const query_options& options, std::optional<service::group0_guard> guard) const {
    ...
    if (strong_consistency::is_strongly_consistent(db.as_data_dictionary(), keyspace())) {
        ...
        try {
            co_await qp.wait_for_table_raft_groups_on_all_hosts(cf.schema()->id(), lowres_clock::now() + std::chrono::seconds(60));
        } catch (...) {
            mylogger.warn("Failed to wait for raft groups of {}.{} to start on all hosts: {}", keyspace(), column_family(), std::current_exception());
        }
    }
