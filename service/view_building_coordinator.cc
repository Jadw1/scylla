...
#include "service/view_building_mutation.hh"
...
future<> view_building_coordinator::commit_mutations(group0_guard guard, utils::chunked_vector<mutation> mutations, std::string_view description) {
...
future<> view_building_coordinator::mark_view_build_status_started(const group0_guard& guard, table_id view_id, utils::chunked_vector<mutation>& out) {
...
future<> view_building_coordinator::mark_all_remaining_view_build_statuses_started(const group0_guard& guard, table_id base_id, utils::chunked_vector<mutation>& out) {
...
future<> view_building_coordinator::update_views_statuses(const group0_guard& guard, table_id base_id, utils::chunked_vector<mutation>& out) {
    if (!_vb_sm.views_state.views_per_base.contains(base_id)) {
        co_return;
    }
...
future<utils::chunked_vector<mutation>> view_building_coordinator::start_tasks(const group0_guard& guard, std::vector<utils::UUID> tasks) {
...
future<utils::chunked_vector<mutation>> view_building_coordinator::update_state_after_work_is_done(const group0_guard& guard, const locator::tablet_replica& replica, view_building_coordinator::remote_work_results results) {
...
void view_building_coordinator::generate_tablet_migration_updates(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, const locator::tablet_map& tmap, locator::global_tablet_id gid, const locator::tablet_transition_info& trinfo) {
...
void view_building_coordinator::generate_tablet_resize_updates(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id, const locator::tablet_map& old_tmap, const locator::tablet_map& new_tmap) {
...
void view_building_coordinator::abort_tasks(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id) {
    if (!_vb_sm.building_state.tasks_state.contains(table_id)) {
        return;
    }
    ...
    view_building_task_mutation_builder builder(guard.write_timestamp());
    auto abort = [&] (const task_map& task_map) {
    ...
    for (auto& [_, replica_tasks]: _vb_sm.building_state.tasks_state.at(table_id)) {
        for (auto& [_, building_task_map]: replica_tasks.view_tasks) {
            abort(building_task_map);
        }
        abort(replica_tasks.staging_tasks);
    }

    out.emplace_back(builder.build());
}

void view_building_coordinator::abort_tasks(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id, locator::tablet_replica replica, dht::token last_token) {
...
void view_building_coordinator::rollback_aborted_tasks(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id) {
    if (!_vb_sm.building_state.tasks_state.contains(table_id)) {
        return;
    }
...
void view_building_coordinator::rollback_aborted_tasks(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id, locator::tablet_replica replica, dht::token last_token) {
